...
Если обновление содержит другия файлы, кроме исполняемого, их нужно запаковать в zip-архив, с сохранением структуры каталогов и так же положить его рядом с **`ftpupdater.exe`**. Имя архива должно совпадать с именем исполняемого файла (пример: **`app.exe`** и **`app.zip`**). Сам исполняемый файл в архив лучше не запаковывать.

Запустить **`ftpupdater.exe --mkey "R%Q480WMofRwn16L"`**

Полученный **`manifest.json`** загружается на сервер вместе с исполнямеый файлом и zip-архивом.
