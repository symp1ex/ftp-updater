...
В **`output.txt`** будет содержаться логин\пароль от FTP-сервера и `url`-адрес HTTP-хранилища в зашифрованном виде. Их нужно получить только один раз, учётные данные перед запуском скрипта необходимо прописать в конфиге **`fu-tools.json`** 
...
```
Скрипт присутствует в архиве с релизом в виде исполняемого `.exe`-файла
